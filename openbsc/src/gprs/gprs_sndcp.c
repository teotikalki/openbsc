...
static int defrag_segments(struct gprs_sndcp_entity *sne)
{
	...
	rc = gprs_sndcp_hdrcomp_expand(msg->data, msg->len, sne->pcomp);
	sne->pcomp = 0;
	if (rc < 0)
		return -EIO;
	else
			 ...
			 uint8_t *hdr, uint16_t len)
{
	...
	rc = gprs_sndcp_hdrcomp_expand(npdu, npdu_len, sne->pcomp);
	if (rc < 0)
		return -EIO;
	else
		npdu_len = rc;
...
int sndcp_sn_xid_ind(struct gprs_llc_xid_field *xid_field_indication, struct gprs_llc_xid_field *xid_field_response, struct gprs_llc_lle *lle)
{
	...
	if(rc >= 0)
	{
		...
		/* FIXME: Do something useful here, other then rejecting everything! */
