...
static int gprs_sndcp_pcomp_test_ind(uint8_t * packet, int packet_len);
static int gprs_sndcp_pcomp_test_req(uint8_t * packet, int packet_len);
#endif
			      ...
			      const struct llist_head *comp_entities)
{
	...
	/* Find pcomp_index */
	pcomp_index =
	    gprs_sndcp_comp_get_idx(comp_entity,
							   pcomp);

#if GPRS_SNDCP_HDRCOMP_RFC1144_TEST == 1
	/* Test mode */
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_expand(): testing compression...!\n");
	rc = gprs_sndcp_pcomp_test_ind(packet, packet_len);
#else
	/* Normal operation: */
				...
				int nsapi)
{
	...
	/* Find out which compression entity handles the packet */
	comp_entity =
	    gprs_sndcp_comp_by_nsapi(comp_entities, nsapi);
	...
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_expand(): testing compression...!\n");
	rc = gprs_sndcp_pcomp_test_req(packet, packet_len);
	*pcomp = 0;
	return rc;
#else
	/* Normal operation: */
	...
	/* Find pcomp value */
	*pcomp =
	    gprs_sndcp_comp_get_comp(comp_entity,
							   pcomp_index);
...
static int gprs_sndcp_pcomp_test_check_packet(uint8_t * packet,
						...
						packet_len_uncompressed)
{
	...
	if (packet_len != packet_len_uncompressed) {
		LOGP(DSNDCP, LOGL_INFO,
		     "prs_sndcp_pcomp_test_check_packet(): Error: Packet length mismatch!\n");
	...
	if (memcmp(packet, packet_backup, packet_len)) {
		LOGP(DSNDCP, LOGL_INFO,
		     "prs_sndcp_pcomp_test_check_packet(): Warning: Packet content!\n");
		LOGP(DSNDCP, LOGL_INFO,
		     "prs_sndcp_pcomp_test_check_packet(): %s\n",
		...
		LOGP(DSNDCP, LOGL_INFO,
		     "prs_sndcp_pcomp_test_check_packet():  %04x\n",
		     checksum);

		if (checksum == 0x0000) {
			LOGP(DSNDCP, LOGL_INFO,
			     "prs_sndcp_pcomp_test_check_packet(): Checksum looks good!\n");
			...
			if (memcmp
				...
				LOGP(DSNDCP, LOGL_INFO,
				     "prs_sndcp_pcomp_test_check_packet(): Packet looks also good!\n");
		} else {
			test_errors++;
#if GPRS_SNDCP_HDRCOMP_RFC1144_TEST_EXITONERR == 1
			exit(1);
...
static int gprs_sndcp_pcomp_test_ind(uint8_t * packet, int packet_len)
{
	...
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_ind(): packet_len=%i\n", packet_len);
	packet_backup = talloc_zero_size(NULL, packet_len);
	memcpy(packet_backup, packet, packet_len);

	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_ind(): IND:          %s\n",
	     osmo_hexdump_nospc(packet, packet_len));
	packet_len_compressed =
	    gprs_sndcp_pcomp_rfc1144_compress(test_compression_state_rx,
						packet, packet_len,
						&pcomp);
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_ind(): IND (COMP):   %s\n",
	     osmo_hexdump_nospc(packet, packet_len_compressed));
	packet_len_uncompressed =
	    gprs_sndcp_pcomp_rfc1144_expand(test_compression_state_tx,
					      packet,
					      packet_len_compressed,
					      pcomp);
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_ind(): IND (DECOMP): %s\n",
	     osmo_hexdump_nospc(packet, packet_len_uncompressed));
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_ind(): packet_len=%i   packet_len_compressed=%i   packet_len_uncompressed=%i\n",
	     packet_len, packet_len_compressed, packet_len_uncompressed);

	gprs_sndcp_pcomp_test_check_packet(packet, packet_backup,
					     packet_len,
					     packet_len_uncompressed);
	talloc_free(packet_backup);
	gprs_sndcp_pcomp_rfc1144_stat(test_compression_state_rx);
	gprs_sndcp_pcomp_rfc1144_stat(test_compression_state_tx);
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_ind(): Test errors: %i\n",
	     test_errors);
	return packet_len;
}
...
static int gprs_sndcp_pcomp_test_req(uint8_t * packet, int packet_len)
{
	...
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_req(): packet_len=%i\n", packet_len);
	packet_backup = talloc_zero_size(NULL, packet_len);
	memcpy(packet_backup, packet, packet_len);

	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_req(): REQ:          %s\n",
	     osmo_hexdump_nospc(packet, packet_len));
	packet_len_compressed =
	    gprs_sndcp_pcomp_rfc1144_compress(test_compression_state_tx,
						packet, packet_len,
						&pcomp);
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_req(): REQ (COMP):   %s\n",
	     osmo_hexdump_nospc(packet, packet_len_compressed));
	packet_len_uncompressed =
	    gprs_sndcp_pcomp_rfc1144_expand(test_compression_state_rx,
					      packet,
					      packet_len_compressed,
					      pcomp);
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_req(): REQ (DECOMP): %s\n",
	     osmo_hexdump_nospc(packet, packet_len_uncompressed));
	LOGP(DSNDCP, LOGL_INFO,
	     "gprs_sndcp_pcomp_test_req(): packet_len=%i   packet_len_compressed=%i   packet_len_uncompressed=%i\n",
	     packet_len, packet_len_compressed, packet_len_uncompressed);
